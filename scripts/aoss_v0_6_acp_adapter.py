...
def classify_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "has_unmeasured_authority": all(event.authorization_state == "UNMEASURED" for event in events),
        "has_unmeasured_validation": all(event.validation_state == "UNMEASURED" for event in events),
