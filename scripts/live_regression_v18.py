...
def assert_response(turn: int, category: str, status_code: int, body: dict, request_payload: dict) -> list[str]:
    ...
    if category == "adversarial":
        ...
        if "reason" not in body or not str(body.get("reason", "")).strip():
...
def run() -> None:
    ...
    with httpx.Client(timeout=TIMEOUT, headers=request_headers()) as client:
        ...
        for turn in range(1, 31):
            ...
            print(f"  {marker} T{turn:02d} | {category:<11} | HTTP={response.status_code} | decision={body.get('decision', '—'):<7} | {latency_ms:.0f}ms")
    ...
    Path("regression_results_v18.json").write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    ...
    print(f"  Audit baseline: status={audit.get('status')} version={audit.get('version')} axiom_count={audit.get('axiom_count')} turn_count={audit.get('turn_count')}")
    if all_errors:
