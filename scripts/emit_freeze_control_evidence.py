...
def git(*args: str) -> str:
    result = subprocess.run(["git", *args], cwd=ROOT, check=True, capture_output=True, text=True, timeout=30)
    return result.stdout.strip()


def git_show_bytes(commit_sha: str, path: str) -> bytes:
    result = subprocess.run(["git", "show", f"{commit_sha}:{path}"], cwd=ROOT, check=True, capture_output=True, timeout=30)
...
def pip_freeze_digest() -> tuple[str, list[str]]:
    result = subprocess.run([sys.executable, "-m", "pip", "freeze"], cwd=ROOT, check=True, capture_output=True, text=True, timeout=60)
    lines = sorted(line.strip() for line in result.stdout.splitlines() if line.strip())
    return sha256_bytes("\n".join(lines).encode()), lines


def pip_version() -> str:
    result = subprocess.run([sys.executable, "-m", "pip", "--version"], cwd=ROOT, check=True, capture_output=True, text=True, timeout=10)
    return result.stdout.strip()
...
def main() -> int:
    ...
    target_candidate_sha = require_full_sha(os.getenv("PDMAL_TARGET_CANDIDATE_SHA", ""), "PDMAL_TARGET_CANDIDATE_SHA")
    git("cat-file", "-e", f"{target_candidate_sha}^{{commit}}")
    target_source_sha256 = {path: sha256_bytes(git_show_bytes(target_candidate_sha, path)) for path in TARGET_PATHS}
    ...
    if target_lock_sha != current_lock_sha:
        raise RuntimeError("verifier dependency lock differs from target candidate lock; refusing to emit E2b evidence")
    verifier_workflow_sha = sha256_file(ROOT / ".github/workflows/governance-ci.yml")
    freeze_state = observed_negative_state()
    freeze_state["state_hash"] = sha256_bytes(canonical_bytes(freeze_state))
    ...
    negative_document = {
        ...
        "interpretation": {"empirical_n_observed_in_workspace": 0, "pilot_authorization": "NOT_GRANTED", "freeze": "NOT_CREATED", "unblinding": "NOT_PERFORMED_IN_THIS_JOB"},
        "limitations": [
            "This artifact proves observed negative-state conditions in the current verification job and workspace; it is not a retrospective proof that no pilot execution ever occurred elsewhere.",
    ...
    print(json.dumps({"verifier_commit_sha": verifier_commit_sha, "target_candidate_sha": target_candidate_sha, "e2b_artifact": str(fingerprint_path), "m6_artifact": str(negative_path), "manifest": str(manifest_path)}, sort_keys=True))
    return 0
