...
ARTIFACT_PROFILE = "PDMAL_AUTHORIZED_PILOT_V1"
REQUIRED_RECORD_FIELDS = {
    ...
    "runtime_ms", "status", "excluded", "exclusion_reason", "environment_fingerprint",
    "artifact_sha256",
}
...
def validate_artifact(document: Mapping[str, Any], *, expected_seed: int | None = None) -> None:
    required = {
        ...
        "empirical_data_collection", "frozen_commit_sha", "seed_id", "runtime_seconds", "records",
    }
    ...
    if document["artifact_profile"] != ARTIFACT_PROFILE:
        raise AssertionError("artifact_profile does not identify the authorized pilot contract")
    ...
    if not isinstance(document["frozen_commit_sha"], str) or len(document["frozen_commit_sha"]) != 40:
