...
import run_track_a_epoch_001 as r
...
def preflight():
    return r.expected_preflight(candidate_sha=CANDIDATE, candidate_tree_sha=TREE)
...
def verification():
    return r.expected_verification(candidate_sha=CANDIDATE, closure_blob_sha=CLOSURE_BLOB)
...
def test_exact_successor_records_accept():
    r.validate_preflight(preflight(), candidate_sha=CANDIDATE, candidate_tree_sha=TREE)
    r.validate_freeze(
        freeze(), candidate_sha=CANDIDATE, candidate_tree_sha=TREE,
        preflight_blob_sha=PREFLIGHT_BLOB, protected_source_blobs=source_blobs(),
    )
    r.validate_closure(closure(), candidate_sha=CANDIDATE, freeze_blob_sha=FREEZE_BLOB)
    r.validate_verification(verification(), candidate_sha=CANDIDATE, closure_blob_sha=CLOSURE_BLOB)
    r.validate_collection_authorization(
        auth(), authorization_parent_sha=PARENT, candidate_sha=CANDIDATE, candidate_tree_sha=TREE,
        preflight_blob_sha=PREFLIGHT_BLOB, freeze_blob_sha=FREEZE_BLOB,
        closure_blob_sha=CLOSURE_BLOB, verification_blob_sha=VERIFY_BLOB,
    )


@pytest.mark.parametrize("field,value", [
    ("record_type", "WRONG"), ("schema_version", 1), ("protocol_id", "WRONG"),
    ("epoch_id", "TRACK_A_EPOCH_004"), ("authorization_parent_sha", "9" * 40),
    ("frozen_candidate_sha", "8" * 40), ("frozen_candidate_tree_sha", "7" * 40),
    ("preflight_blob_sha", "6" * 40), ("freeze_manifest_blob_sha", "5" * 40),
    ("closure_packet_blob_sha", "4" * 40), ("verification_classification_blob_sha", "3" * 40),
    ("preregistration_merge_sha", "2" * 40), ("analysis_lock_merge_sha", "1" * 40),
    ("analysis_blob_sha", "0" * 40), ("analysis_config_sha256", "0" * 64),
    ("requirements_lock_blob_sha", "9" * 40), ("algorithm_id", "DGAF"),
    ("seed_start", 20261001), ("seed_end", 20261050), ("seed_count", 49),
    ("expected_observations", 9000), ("authorize_empirical_collection", False),
    ("authorize_unblinding", True), ("historical_pooling_allowed", True),
    ("epoch_004_substitution_allowed", True), ("high_assurance_authorized", True),
])
def test_authorization_mutations_fail_closed(field, value):
    ...
    with pytest.raises(SystemExit):
        r.validate_collection_authorization(
            data, authorization_parent_sha=PARENT, candidate_sha=CANDIDATE, candidate_tree_sha=TREE,
            preflight_blob_sha=PREFLIGHT_BLOB, freeze_blob_sha=FREEZE_BLOB,
            closure_blob_sha=CLOSURE_BLOB, verification_blob_sha=VERIFY_BLOB,
        )
...
def test_authorization_shape_mutations_fail_closed(which):
    ...
    if which == "missing":
        ...
        data["legacy_epoch_004_observations"] = True
    with pytest.raises(SystemExit):
        r.validate_collection_authorization(
            data, authorization_parent_sha=PARENT, candidate_sha=CANDIDATE, candidate_tree_sha=TREE,
            preflight_blob_sha=PREFLIGHT_BLOB, freeze_blob_sha=FREEZE_BLOB,
...
@pytest.mark.parametrize("seed", [20261001, 20270100, 20270151, 0, -1])
...
def test_keyed_order_changes_with_key():
    assert r.ordered_matrix_cells(r.SEEDS[0], b"x" * 32) != r.ordered_matrix_cells(r.SEEDS[0], b"y" * 32)


def test_topology_labels_are_keyed_unique_nonplaintext():
    labels = [r.blind_topology(t, b"x" * 32) for t in r.TOPOLOGIES]
    assert len(set(labels)) == 5
    assert all(x.startswith("topology_") for x in labels)
...
@pytest.mark.parametrize("mutation", [
    "duplicate", "missing", "extra", "wrong_seed", "wrong_protocol", "wrong_algorithm",
    "malformed_blind", "wrong_failure", "nonbool_endpoint", "excluded", "extra_key", "missing_key",
])
...
@pytest.mark.parametrize("record_kind,field,value", [
...
def test_successor_gate_mutations_fail_closed(record_kind, field, value):
    if record_kind == "preflight":
        data = preflight(); data[field] = value
        fn = lambda: r.validate_preflight(data, candidate_sha=CANDIDATE, candidate_tree_sha=TREE)
    elif record_kind == "freeze":
        data = freeze(); data[field] = value
        fn = lambda: r.validate_freeze(
            data, candidate_sha=CANDIDATE, candidate_tree_sha=TREE,
            preflight_blob_sha=PREFLIGHT_BLOB, protected_source_blobs=source_blobs(),
        )
    elif record_kind == "closure":
        data = closure(); data[field] = value
        fn = lambda: r.validate_closure(data, candidate_sha=CANDIDATE, freeze_blob_sha=FREEZE_BLOB)
    else:
        data = verification(); data[field] = value
        fn = lambda: r.validate_verification(data, candidate_sha=CANDIDATE, closure_blob_sha=CLOSURE_BLOB)
    with pytest.raises(SystemExit):
        fn()


@pytest.mark.parametrize("parents,changed", [
    ((), (str(r.AUTH_PATH),)),
...
def test_authorization_commit_shape_fail_closed(monkeypatch, parents, changed):
    ...
    monkeypatch.setattr(r, "git_path_history", lambda path, revision="HEAD": ("9" * 40,))
    with pytest.raises(SystemExit):
...
def test_authorization_commit_shape_accepts(monkeypatch):
    ...
    monkeypatch.setattr(r, "git_changed_paths", lambda revision="HEAD": (str(r.AUTH_PATH),))
    monkeypatch.setattr(r, "git_path_exists_at", lambda path, revision: False)
    monkeypatch.setattr(r, "git_path_history", lambda path, revision="HEAD": ("9" * 40,))
    assert r.validate_authorization_commit_shape() == ("9" * 40, PARENT)


def test_authorization_existing_file_fails_closed(monkeypatch):
    monkeypatch.setattr(r, "git_head", lambda: "9" * 40)
    monkeypatch.setattr(r, "git_parents", lambda revision="HEAD": (PARENT,))
    monkeypatch.setattr(r, "git_changed_paths", lambda revision="HEAD": (str(r.AUTH_PATH),))
    monkeypatch.setattr(r, "git_path_exists_at", lambda path, revision: True)
    with pytest.raises(SystemExit):
        r.validate_authorization_commit_shape()


def test_authorization_history_must_equal_head(monkeypatch):
    monkeypatch.setattr(r, "git_head", lambda: "9" * 40)
    monkeypatch.setattr(r, "git_parents", lambda revision="HEAD": (PARENT,))
    monkeypatch.setattr(r, "git_changed_paths", lambda revision="HEAD": (str(r.AUTH_PATH),))
    monkeypatch.setattr(r, "git_path_exists_at", lambda path, revision: False)
    monkeypatch.setattr(r, "git_path_history", lambda path, revision="HEAD": ("8" * 40,))
    with pytest.raises(SystemExit):
        r.validate_authorization_commit_shape()
