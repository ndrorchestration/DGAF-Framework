...
def preflight():
    return r.expected_preflight(candidate_sha=CANDIDATE, candidate_tree_sha=TREE, custody=CUSTODY)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("custody_encrypted_private_key_sha256", "9" * 64),
...
def test_authorization_mutations_fail_closed(field, value):
    data = auth()
    assert data[field] != value, f"negative control must mutate {field}"
    data[field] = value
...
@pytest.mark.parametrize("seed", [20270101, 20270150, 20270200, 20270251, 20261001, 0, -1])
...
def test_successor_gate_mutations_fail_closed(record_kind, field, value):
    if record_kind == "preflight":
        ...
        def validate():
            r.validate_closure(data, candidate_sha=CANDIDATE, freeze_blob_sha=FREEZE_BLOB)
        ...
        def validate():
            r.validate_verification(data, candidate_sha=CANDIDATE, closure_blob_sha=CLOSURE_BLOB)
