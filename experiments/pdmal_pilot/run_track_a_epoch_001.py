#!/usr/bin/env python3
"""Fail-closed Track A Epoch 001 collection runner.

Code presence is not authorization. Empirical execution requires a completed
precollection preflight, immutable freeze, closure/verification chain, and a
separate one-file collection-authorization commit. PR validation cannot satisfy
that predicate and must never execute the preregistered scientific panel.
"""
...
PREFLIGHT_PATH = RUNS_ROOT / "TRACK_A_EPOCH_001_PRECOLLECTION_PREFLIGHT.json"
FREEZE_PATH = RUNS_ROOT / "TRACK_A_EPOCH_001_IMMUTABLE_FREEZE_MANIFEST.json"
CLOSURE_PATH = RUNS_ROOT / "TRACK_A_EPOCH_001_FINAL_CLOSURE_PACKET.json"
VERIFICATION_PATH = RUNS_ROOT / "TRACK_A_EPOCH_001_VERIFICATION_CLASSIFICATION.json"
AUTH_PATH = RUNS_ROOT / "TRACK_A_EPOCH_001_COLLECTION_AUTHORIZATION.json"

PREREG_PATH = "docs/experiment/TRACK_A_TOPOLOGY_ROBUSTNESS_EPOCH_001_PREREGISTRATION.json"
ANALYSIS_LOCK_PATH = "docs/experiment/TRACK_A_EPOCH_001_ANALYSIS_LOCK.json"
ANALYSIS_PATH = "experiments/pdmal_pilot/track_a_epoch_001_analysis.py"
...
RUNNER_PATH = "experiments/pdmal_pilot/run_track_a_epoch_001.py"
...
ANALYSIS_CONFIG_SHA256 = "355b164f69e91405819f092d0721b7597b87b06de79394a0c451169410a5ab6d"
...
PUBLIC_RECORD_KEYS = frozenset({
    "protocol_id", "algorithm_id", "frozen_candidate_sha", "seed_id",
    "blinded_topology_id", "failure_count", "ffcr_success", "excluded",
})
PROHIBITED_PUBLIC_KEYS = frozenset({
    "topology", "topology_name", "topology_fingerprint", "mapping", "blind_key",
    "blinding_key", "dgaf", "full_dgaf", "canonical_dgaf", "condition",
})
...
def require_immutable_gate_history(candidate_sha: str, authorization_parent: str) -> None:
    ...
    for path in gate_paths:
        ...
        if len(history) != 1:
            raise SystemExit(
                f"Track A collection prohibited: {path} must have exactly one immutable history commit; "
    ...
    chain = (
        ...
        (verification_commit, authorization_parent, "verification->authorization-parent"),
    )
...
def expected_preflight(*, candidate_sha: str, candidate_tree_sha: str) -> dict:
    return {
...
def validate_preflight(data: dict, *, candidate_sha: str, candidate_tree_sha: str) -> None:
    require_exact_object(data, expected_preflight(candidate_sha=candidate_sha, candidate_tree_sha=candidate_tree_sha), "preflight")


def expected_freeze(*, candidate_sha: str, candidate_tree_sha: str, preflight_blob_sha: str,
                    protected_source_blobs: dict[str, str]) -> dict:
    return {
...
def validate_freeze(data: dict, *, candidate_sha: str, candidate_tree_sha: str,
                    preflight_blob_sha: str, protected_source_blobs: dict[str, str]) -> None:
...
def validate_closure(data: dict, *, candidate_sha: str, freeze_blob_sha: str) -> None:
    require_exact_object(data, expected_closure(candidate_sha=candidate_sha, freeze_blob_sha=freeze_blob_sha), "closure packet")
...
def expected_authorization(*, authorization_parent_sha: str, candidate_sha: str, candidate_tree_sha: str,
                           preflight_blob_sha: str, freeze_blob_sha: str, closure_blob_sha: str,
                           verification_blob_sha: str) -> dict:
    return {
...
def validate_collection_authorization(data: dict, **bindings: object) -> None:
    require_exact_object(data, expected_authorization(**bindings), "authorization")
...
def validate_authorization_commit_shape() -> tuple[str, str]:
    ...
    if changed != (str(AUTH_PATH),):
        raise SystemExit(
            "Track A collection prohibited: authorization commit must introduce exactly one file "
            f"{AUTH_PATH}; changed={list(changed)}"
        )
    if git_path_exists_at(str(AUTH_PATH), parents[0]):
        raise SystemExit("Track A collection prohibited: authorization file already existed before authorization HEAD")
...
def validate_matrix_records(records: Sequence[dict], *, seed: int) -> None:
    ...
    for blinded in blinded_ids:
        if {failure for topo, failure in cells if topo == blinded} != expected_failures:
...
def ordered_matrix_cells(seed: int, key: bytes) -> tuple[tuple[str, int], ...]:
    ...
    def rank(cell: tuple[str, int]) -> bytes:
        ...
        msg = f"order:{seed}:{topology}:{failure}".encode("utf-8")
...
def require_collection_authorization() -> tuple[str, bytes, Path, Path]:
    ...
    if os.getenv("PDMAL_TRACK_A_EPOCH_001_AUTHORIZED") != "1":
        raise SystemExit("Track A collection prohibited: explicit Track A authorization env required")
    ...
    validate_preflight(preflight, candidate_sha=candidate_sha, candidate_tree_sha=candidate_tree)
    validate_freeze(
        freeze, candidate_sha=candidate_sha, candidate_tree_sha=candidate_tree,
        preflight_blob_sha=preflight_blob, protected_source_blobs=frozen_sources,
    )
    ...
    return candidate_sha, secret.encode(), public_root, protected_root


def collect_seed(seed: int, *, frozen_sha: str, key: bytes) -> tuple[dict, dict, dict]:
    started = monotonic()
    public_records: list[dict[str, object]] = []
    mapping = {blind_topology(t, key): t for t in TOPOLOGIES}
    for topology, failure_count in ordered_matrix_cells(seed, key):
        ...
        if result.attempt_status is not AttemptStatus.SUCCESS:
            raise SystemExit(
                f"Track A collection prohibited: trial failed seed={seed} topology={topology} failure_count={failure_count}"
            )
    ...
    public_doc = {
        "record_type": "TRACK_A_EPOCH_001_BLINDED_SEED_DATASET",
        "schema_version": 2,
        "protocol_id": PROTOCOL_ID,
        "seed_id": seed,
    ...
    protected_mapping_doc = {
        "record_type": "TRACK_A_EPOCH_001_PROTECTED_TOPOLOGY_MAPPING",
        "schema_version": 2,
        "protocol_id": PROTOCOL_ID,
        "seed_id": seed,
        "mapping": mapping,
    ...
    protected_key_custody_doc = {
        "record_type": "TRACK_A_EPOCH_001_PROTECTED_BLINDING_KEY_CUSTODY",
        "schema_version": 1,
        "protocol_id": PROTOCOL_ID,
...
def run_epoch() -> int:
    frozen_sha, key, public_root, protected_root = require_collection_authorization()
    key_custody_written = False
    manifest_rows: list[dict[str, object]] = []
    for seed in SEEDS:
        public_doc, mapping_doc, key_doc = collect_seed(seed, frozen_sha=frozen_sha, key=key)
        public_path = public_root / f"track_a_epoch_001_seed_{seed}.json"
        mapping_path = protected_root / f"track_a_epoch_001_mapping_{seed}.json"
        ...
        if not key_custody_written:
            write_json_with_sidecar(protected_root / "track_a_epoch_001_blinding_key_custody.json", key_doc)
    ...
    if sum(int(row["record_count"]) for row in manifest_rows) != EXPECTED_TOTAL:
        raise SystemExit("Track A collection prohibited: whole-epoch matrix incomplete")
    manifest = {
        "record_type": "TRACK_A_EPOCH_001_BLINDED_COLLECTION_MANIFEST",
        "schema_version": 2,
        "protocol_id": PROTOCOL_ID,
        "frozen_candidate_sha": frozen_sha,
        "seed_count": len(SEEDS),
    ...
    write_json_with_sidecar(public_root / "track_a_epoch_001_manifest.json", manifest)
    print(f"TRACK_A_EPOCH_001_COLLECTION_COMPLETE: {len(SEEDS)} seeds; {EXPECTED_TOTAL} observations")
    return 0


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--execute", action="store_true", help="execute only after every authorization predicate passes")
