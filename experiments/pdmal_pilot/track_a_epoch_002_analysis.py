"""Locked primary analysis for Track A topology-robustness Epoch 002.

Consumes an already authorized, structurally locked, unblinded Track A successor
...
def paired_seed_effects(records: Sequence[Mapping[str, object]]) -> list[PairedSeedEffect]:
    ...
    for seed in SEEDS:
        pdmal_values = [observed[(seed, PRIMARY_TOPOLOGY, failure_count)] for failure_count in FAILURE_COUNTS]
        rr_values = [observed[(seed, PRIMARY_COMPARATOR, failure_count)] for failure_count in FAILURE_COUNTS]
        effects.append(
            PairedSeedEffect(
                seed=seed,
