...
The materialization chain has now executed through separate governed events: PR #824 admitted the canonical non-secret materialization evidence and PR #826 established the immutable creation-only `MATERIALIZATION_RECEIPT`. PR #831 accepted fail-closed local locked-analysis runner tooling, PR #828 accepted bounded `LOCKED_PRIMARY_ANALYSIS_ONLY` authorization, and PR #835 accepted non-executing content-addressed result-admission tooling. The real primary analysis remains **NOT RUN** and no locked result is established.
