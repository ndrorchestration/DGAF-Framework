...
def _ingest_seed_tokens(scpe: StructuralContextPruningEngine) -> None:
    ...
    for tid, content, tier, trust in seeds:
        scpe.ingest(ContextToken(token_id=tid, content=content, tier=tier, inserted_at=t0, has_trust_edge=trust))
    ...
    for i, rec in enumerate(audit_log, 1):
        ...
        trajectory.append(
            dict(
                ...
                phi_checkpoint_passed=int(rec.phi_checkpoint_passed) if rec.phi_checkpoint_passed is not None else "",
            )
        )
    return trajectory
...
def run_simulation(output_dir: Path = Path(".")) -> None:
    ...
    print("=" * 80)
    print(
        f"{'Turn':>5} {'ID':>5} {'KAPPA':>12} {'EffConf':>8} {'SLock':>6} "
    ...
    print("\n  PER-CATEGORY SUMMARY")
    print(f"  {'Category':>14} {'N':>4} {'AvgEff':>8} {'SLockR':>8} " f"{'GoldStR':>8} {'HPGbyp':>8} {'KillR':>8}")
    print("  " + "-" * 62)
    cat_rows = []
    ...
    for pt in phi_traj:
        ...
        print(f"  {pt['turn']:>5} {pt['ratio']:>7.4f} {pt['phi_delta']:>8.4f} " f"{chkpt:>7} {passed:>5}")
    ...
    assert dgaf_kills_total == 3, f"Expected 3 DGAF kills (turns 16-18), got {dgaf_kills_total}"
    print(f"  [PASS] DGAF kills = {dgaf_kills_total} (expected 3)")

    # All HPG-applied sequential turns must have eff_conf >= floor
    seq_applied = [r for r in amethyst.audit_log if r.kappa_category == "sequential" and r.hpg_applied]
    for r in seq_applied:
        assert (
            r.hpg_effective_confidence >= SEQUENTIAL_CONF_FLOOR
    ...
    for r in step_locked_recs:
        assert r.kappa_category == "sequential", f"{r.turn_id} step_locked but kappa={r.kappa_category}"
    ...
    for row in fan_rows:
        assert row["hpg_eff_conf"] >= row["input_confidence"], f"{row['turn_id']} fan_out eff_conf < input_conf"
    ...
    assert len(axiom_prunes) == 0, f"T0 GUARD BREACH: {len(axiom_prunes)} axiom tokens pruned"
