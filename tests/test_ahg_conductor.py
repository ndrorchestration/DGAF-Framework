...
class Test3DPhaseSpace:
    ...
    def test_high_d_explore_high_exploration(self):
        assert (
            compute_3d_phase(StateVector(D_explore=0.9, K=0.1))[0]
            > compute_3d_phase(StateVector(D_explore=0.1, K=0.9))[0]
        )

    def test_high_k_low_exploration(self):
        assert compute_3d_phase(StateVector(D_explore=0.0, K=1.0))[0] < 0.01

    def test_high_d_e_high_dissent(self):
        assert (
            compute_3d_phase(StateVector(D_e=0.9, D_correct=0.5, K=0.1))[1]
            > compute_3d_phase(StateVector(D_e=0.1, D_correct=0.1, K=0.9))[1]
        )
    ...
    def test_uncertainty_clipped_at_one(self):
        assert compute_3d_phase(StateVector(C=1.0, R=1.0))[2] <= 1.0
...
class TestPhaseIntentV14:
    ...
    def test_intent_tribunal_active_false_for_low_phi(self, conductor, zero_sv):
        assert conductor.step(zero_sv).tribunal_active is False

    def test_intent_tribunal_active_true_for_tension(self):
        c = AHGConductor(hysteresis_turns=1)
        intent = c.step(StateVector(D_e=10.0, N=10.0, C=10.0, R=10.0))
    ...
    def test_intent_message_is_string(self, conductor, zero_sv):
        intent = conductor.step(zero_sv)
        assert isinstance(intent.message, str) and len(intent.message) > 0

    def test_intent_message_contains_phi(self, conductor, zero_sv):
        assert "φ=" in conductor.step(zero_sv).message
...
class TestStateVector:
    def test_d_p_is_sum(self):
        assert math.isclose(StateVector(D_explore=0.3, D_correct=0.2).D_p, 0.5, rel_tol=1e-9)
...
class TestAHGSidecarV14:
    ...
    def test_herald_callback_receives_enriched_intent(self, sidecar):
        ...
        assert (
            intent.phase_exploration is not None
            and intent.phase_dissent is not None
            and intent.phase_uncertainty is not None
        )
