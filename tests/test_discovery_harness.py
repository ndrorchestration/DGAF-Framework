...
def test_critical_mutations_only_change_ephemeral_copies():
    source = {"authorization":"NOT_AUTHORIZED","scientific_n":0,"decision":"UNKNOWN",
...
def test_control_interaction_detects_collisions():
    controls = (
        ControlContract("A", writes=frozenset({"state"}), requires=frozenset({"ready"})),
        ControlContract("B", reads=frozenset({"state"}), writes=frozenset({"state"}), forbids=frozenset({"ready"})),
    )
