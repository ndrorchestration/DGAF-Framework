...
def test_b123_nonempirical_adjudication_families_are_catalog_mapped_without_authorization():
    catalog = load_catalog(CATALOG_PATH)
    unmapped = _collect_unmapped_workflows(REPO_ROOT, catalog)
    expected = {
        "AUD-CI-B1-NONEMPIRICAL-ADJUDICATION": ".github/workflows/b1-integration-adjudication.yml",
        "AUD-CI-B2-NONEMPIRICAL-ADJUDICATION": ".github/workflows/b2-integration-adjudication.yml",
        "AUD-CI-B3-NONEMPIRICAL-ADJUDICATION": ".github/workflows/b3-integration-adjudication.yml",
    }
...
def test_b123_nonempirical_profile_families_are_catalog_mapped_with_identity_boundary():
    catalog = load_catalog(CATALOG_PATH)
    unmapped = _collect_unmapped_workflows(REPO_ROOT, catalog)
    expected = {
        "AUD-CI-B1-NONEMPIRICAL-PROFILE": ".github/workflows/b1-semantic-routing-safety-profile.yml",
        "AUD-CI-B2-NONEMPIRICAL-PROFILE": ".github/workflows/b2-stateful-context-closure-profile.yml",
        "AUD-CI-B3-NONEMPIRICAL-PROFILE": ".github/workflows/b3-p33-convergence-profile.yml",
    }
...
def test_branch_disposition_inventory_is_catalog_mapped_without_deletion_authority():
    catalog = load_catalog(CATALOG_PATH)
    unmapped = _collect_unmapped_workflows(REPO_ROOT, catalog)
    audit = next(
        (entry for entry in catalog["audits"] if entry.get("id") == "AUD-CI-BRANCH-DISPOSITION-INVENTORY"),
        None,
    )

    assert audit is not None
    assert ".github/workflows/branch-disposition-inventory.yml" in audit["implementation"]
    assert ".github/workflows/branch-disposition-inventory.yml" not in unmapped
    assert audit["blocking"] is False
    assert audit["independence"] == "SAME_REPOSITORY_NONINDEPENDENT"
    assert any("does not delete" in item.lower() for item in audit["non_effects"])
    assert any("deletion authorization" in item.lower() for item in audit["non_effects"])
