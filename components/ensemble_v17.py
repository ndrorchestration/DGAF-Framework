...
class KAPPARouter:
    ...
    def __init__(self):
        ...
        for candidate in self._KAPPA_SEARCH_PATHS:
            if candidate.exists():
                spec = importlib.util.spec_from_file_location("dynamic_weight_router", candidate)
    ...
    def resolve(self, payload: str, entropy: float = 0.5, kappa_score: float = 0.5) -> Dict:
        ...
        input_data = {"content": payload, "entropy_score": entropy, "kappa_score": kappa_score}
...
class StructuralContextPruningEngine:
    ...
    def prune(self) -> Dict:
        ...
        return dict(
            ...
            **{f"{k.lower()}_count": v for k, v in tc.items()},
        )
...
class PDMALConvergenceMonitor:
    def __init__(
        self, pdmal_graph: PDMALGraph, alert_thresh: float = 0.08, conv_thresh: float = 0.02, n_consec: int = 3
    ):
    ...
    def _cw(self) -> Dict[Tuple[str, str], float]:
        return {(s, d): float(e.weight) for s, targets in self.graph.edges.items() for d, e in targets.items()}
    ...
    def _sev(self, n: int) -> ConvergenceStatus:
        return [ConvergenceStatus.STABLE, ConvergenceStatus.WATCH, ConvergenceStatus.WARN, ConvergenceStatus.ALERT][
            min(n, 3)
        ]
    ...
    def check(self, turn_id: str) -> DivergenceEvent:
        ...
        evt = DivergenceEvent(
            ...
            max_edge=max_edge,
            consecutive_divergent=self._consec_div,
            status=status.code,
...
class HarmonicParametricGate:
    ...
    def gate(self, confidence: float, label: str = "", routing_mode: str = "balanced") -> Dict:
        """
        ...
        entry = dict(
            ...
            adj_conf=round(adj_conf, 6),
            octave=round(octave, 6),
...
class DemiJouleGate:
    ...
    def safety_gate(self, payload: str, tool_call: bool = False, agent_id: str = "unknown") -> Dict:
        payload_lower = payload.lower()
        for p in BLOCKED_PATTERNS:
            if re.search(p, payload_lower):
                return dict(decision="kill", reason="blocked_pattern", axis_scores={}, agent_id=agent_id)
        scores = {}
        ...
        decision = "kill" if mean < self.KILL_THRESHOLD else "reprompt" if mean < self.REPROMPT_THRESHOLD else "pass"
        return dict(
            decision=decision,
...
class ProdigyVerifier:
    ...
    def verify(self, claim: str, confidence: float) -> Dict:
        ...
        return dict(
            mandatory=mandatory, evidence_ok=len(claim.strip()) > 10, confidence=confidence, advisory=not mandatory
        )
...
class ApogeeReviewer:
    ...
    def review(self, confidence: float, artifact_description: str = "") -> Dict:
        ...
        return dict(grade=grade, gold_star=gold_star, confidence=confidence, artifact_description=artifact_description)
...
class AgentAmethyst:
    ...
    def _build_default_pdmal() -> PDMALGraph:
        g = PDMALGraph()
        for node in ["amethyst", "demijoul", "colleen", "prodigy", "apogee", "sentinel_phi"]:
            g.add_node(node)
        for src, dst, w in [
    ...
    def orchestrate_turn(
        ...
        # [2] COLLEEN schema hash
        schema_hash = hashlib.sha256(json.dumps(state, sort_keys=True).encode()).hexdigest()[:12]
        ...
        if pdmal_alert and phi_decision.severity >= PhiDecision.ESCALATE.severity:
            deep = self.demijoul.safety_gate(payload, tool_call=True, agent_id="joint_pdmal_phi_escalation")
        ...
        if hpg_applied:
            hpg_result = self.hpg.gate(confidence, label=tid, routing_mode=hpg_routing_mode)
    ...
    def _early_seal(
        ...
        rec = TurnAuditRecord(
            ...
            dgaf_decision=dgaf_decision,
            phi_decision=phi_code,
            phi_checkpoint_index=None,
            phi_checkpoint_passed=None,
            kappa_category=kappa_category,
            kappa_policy=kappa_policy,
            hpg_routing_mode=kappa_category,
            hpg_applied=False,
            hpg_step_locked=False,
            hpg_effective_confidence=confidence,
            prodigy_advisory=False,
            apogee_grade="N/A",
            gold_star=False,
            scpe_pruned=scpe_stats["pruned"],
            scpe_compression_ratio=scpe_stats["compression_ratio"],
...
if __name__ == "__main__":
    ...
    eng.ingest(ContextToken("ax1", "governance rule", Tier.AXIOM, inserted_at=time.time() - 100))
    eng.ingest(ContextToken("ex1", "cot noise", Tier.EXPLORATORY, inserted_at=time.time() - 100))
    s = eng.prune()
    ...
    for i in range(5):
        scpe2.ingest(ContextToken(f"t{i}", f"content {i}", Tier.OPERATIONAL if i < 3 else Tier.STRUCTURAL))
    ...
    rec = am.orchestrate_turn(
        ...
        entropy_score=0.30,
        kappa_score_hint=0.75,
    )
    assert rec.dgaf_decision == "pass", f"DGAF FAIL: {rec.dgaf_decision}"
    assert rec.kappa_category in ("sequential", "balanced"), f"KAPPA CAT FAIL: {rec.kappa_category}"
    if rec.kappa_category == "sequential":
        assert (
            rec.hpg_step_locked or rec.hpg_effective_confidence >= SEQUENTIAL_CONF_FLOOR
        ), "SEQ FLOOR ORCHESTRATION FAIL"
    print(
        f"[PASS] orchestrate_turn T01 sequential: kappa={rec.kappa_category} "
    ...
    print(
        f"[PASS] orchestrate_turn T02 fan_out: kappa={rec2.kappa_category} " f"eff_conf={rec2.hpg_effective_confidence}"
    )
