...
Version: 1.0.1 | Status: ACTIVE | Agent: Amethyst
...
Pipeline flow:
    raw_eval_batch
        -> [KAPPA select_weights_with_confidence]
...
if _spec is None or _spec.loader is None:
    raise ImportError(f"Unable to load KAPPA router from {_KAPPA_PATH}")
...
KAPPA_VERSION = "3.6"
REPORT_COMPONENT = "DGAF-EVALUATE-ROUTER-v1.0.1"


def apply_weights(score_vector: Dict[str, float], weights: Dict[str, float]) -> float:
    """Compute a weighted composite score from a score vector and weight config."""
...
def run_eval_batch(
    ...
    for result in results:
        policy = result["kappa_policy"]
        category = result["kappa_category"]
