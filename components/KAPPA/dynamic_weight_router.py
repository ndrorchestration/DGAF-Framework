    ...
    if conf >= STRONG_THRESH:
        ...
        for k in base_weights:
            if k in _numeric:
                ...
                if not isinstance(base_value, (int, float)) or not isinstance(balanced_value, (int, float)):
                    raise TypeError(f"non-numeric weight for {k}")
                final_weights[k] = round(alpha * float(base_value) + (1 - alpha) * float(balanced_value), 3)
            else:
