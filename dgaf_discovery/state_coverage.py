...
class TransitionCoverage:
    ...
    def state_coverage(self):
        return (
            self.reached_states / self.specified_states if self.specified_states else 1.0
        )

    @property
    def legal_transition_coverage(self):
        return (
            self.exercised_legal_transitions / self.legal_transitions
    ...
    def forbidden_rejection_coverage(self):
        return (
            self.rejected_forbidden_transitions / self.forbidden_transitions
            if self.forbidden_transitions
            else 1.0
        )
