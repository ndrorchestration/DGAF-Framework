...
| Execution contract | PARTIAL / TGL BLOCKED | Authenticated exact-current-tree P2 evidence pending; TGL/P-35 contract remediation is PR #134 |
...
| TGL contract review | BLOCKED / CURRENT REMEDIATION PR #134 | PR #132/#133 are closed historical lanes; #134 is current-main engineering remediation candidate |
...
The selected remediation is intentionally minimal: restore the established P-35 API and TGL fail-closed behavior, make required/conditional gate semantics explicit, implement deterministic status reduction, make the final seal correspond to the authoritative returned audit state, and expand regression coverage. Broad architectural refactoring is out of scope for the remediation lane.

PR #132 and PR #133 are closed as stale/superseded remediation lanes. Current-main PR #134 carries the correction from the live `main` boundary. It replaces the PPTL CI placeholder with an executable TGL/P-35 suite, strengthens canonical audit sealing, contains terminal P-35/Herald failures, and preserves the non-authorizing experimental boundary. PR #134 must obtain exact-head CI/validation before merge. None of these engineering PRs changes the experimental apparatus identity, creates a freeze, closes P7/P8, grants authorization, or increases empirical N.
...
1. Resolve the TGL/P-35 contract blocker through current-main PR #134 and exact-head validation.
