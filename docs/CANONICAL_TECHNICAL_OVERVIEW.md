...
The repository contains more than one PDMAL/Track-A-era evidence lineage. The legacy PDMAL v0.7.6 protocol remains an active specification/workflow contract for its exact apparatus scope, but its September 6 PRE-FREEZE / Issue #309 candidate-reconstruction state is not the live project-state authority; this September 6 snapshot remains an exact-scope historical source.
