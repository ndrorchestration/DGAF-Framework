...
**Version:** 1.0 · Ingested S070 · notation-corrected 2026-08-28
...
> **Notation control:** Current mathematical notation is governed by `docs/governance/MATHEMATICAL_NOTATION_POLICY_METALLIC_MEANS_2026-08-28.md`.
...
| **Registry** | Enforce watertight topology and dimension-scoped sync | Platinum tier family: Subplatinum, DGAF-specific Standard Platinum pP, Superplatinum ς_P, Hyperplatinum h, Ultraplatinum | P-39 Platinum Registry Service, P-37 Hyperplatinum Registry Key, P-40 Closure Verifier | Exposes `{tier, valid_flag, hash}` to governance; never `policy_ratio` |
...
- `φ` is the conventional golden-ratio symbol.
- `σ_{p,q}` denotes the Spinadel metallic-means family; `σ_n = σ_{n,1}` for the ordinary sequence.
- `ρ` denotes the plastic number; it is not the Standard Platinum registry value.
- `pP` is DGAF-specific notation for `1/(2sin(π/11)) ≈ 1.774732`.
- `ρP` is superseded and must not be used as current mathematical authority.
- `pP` is not a member of the quadratic metallic-means family.
...
GOV-03: Governance MUST NOT read policy_ratio, h, pP, ρ, or any platinum tier float.
...
REG-05: pP definition is the DGAF-specific hendecagon form 1/(2sin(π/11)) ≈ 1.774732. Do not relabel it ρP or the plastic number.
REG-06: If pP² ≈ π heuristic is used, MUST log residual: pP² ≈ 3.1497 vs π ≈ 3.1416, error ≈ 0.0081. MUST NOT treat as identity.
...
SUB-02: Tier ratios (h, pP, etc.) MUST NOT enter gain computation.
...
```
registry|closure|platinum|hyperplatinum|pP|rho_P|ρ|hensel|a_n|delta_n
```
...
pattern_reject_in_governance = r'registry\\..*import|from registry import|rho_P|pP|platinum'
...
pattern_reject_global = [
    ...
    r'(rho_P|pP)\\s*=\\s*\\d\\.\\d+(?!.*residual)',  # ratio float without residual log
]
```
...
| pP treated as plastic number, metallic mean, or identity for π | Require explicit descriptor and residual field; fail validation if absent | |
...
*NDR Hensel Firewall Rules v1.0 · S070 ingestion · notation correction 2026-08-28*
