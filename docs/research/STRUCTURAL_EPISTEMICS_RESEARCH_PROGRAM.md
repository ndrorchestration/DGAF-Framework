...
5. Runtime feedback, containment, rollback, and escalation.
6. Spectral or harmonic methods where the underlying object is genuinely a graph, signal, time series, geometric field, or dynamical system.
...
Can identity, evidence, approval, execution, rollback, and revocation be represented as enforceable state-transition invariants that fail closed at the tool boundary?
...
Specify the state machine and invariants separating evidence, authorization, execution, side effects, rollback, and audit. See `../formalism/AGENT_GOVERNANCE_TRANSITION_SPEC.md`.
