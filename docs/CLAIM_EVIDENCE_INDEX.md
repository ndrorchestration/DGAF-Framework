...
| Evidence-gate toolchain is pinned and fully verified across the supported Python matrix. | `IMPLEMENTED / PENDING VERIFICATION` | `requirements-ci.txt` is pinned and consumed by CI; run `31977296400` passed on 3.10/3.11/3.12 and failed on 3.9 because `pip==26.2.1` requires Python >=3.10. | Pinning is implemented, but the supported-runtime policy and pin set are not yet reconciled. | Full matrix success after closing #51 and rerunning #54. |
