...
- the runtime guard requires production `PDMAL_BLINDING_KEY` to contain at least 32 characters;
- that length check is structural only and is **not** evidence of cryptographic entropy;
...
- the production blinding key structural guard is enforced and the accepted custody evidence separately proves high-entropy CSPRNG generation;
...
| v0.7.6 key-length/domain separation | **DRAFT / UNDER EXACT-HEAD VERIFICATION** | PR #308 runner/tests; key entropy remains a separate custody predicate. |
