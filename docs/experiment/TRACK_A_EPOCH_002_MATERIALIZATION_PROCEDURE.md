...
Status: **TOOLING REFRESH · DATASET LOCK ESTABLISHED · BOUNDED UNBLINDING AUTHORIZED · MATERIALIZATION NOT ESTABLISHED · PRIMARY ANALYSIS NOT AUTHORIZED / NOT RUN · N=0**

Controller: issue #632. This procedure reconstructs the useful tooling-only contract preserved in stale draft #634 onto the current accepted predecessor chain. The accepted dataset-lock event is `e7ba2fe6fc6b3587957c59231da81ae107cacab2`; the accepted bounded unblinding event is `bf6279b9989f211e324ff3e9012788bed95e5c84`. Historical Epoch 001 materialization code is a pattern source only; no Epoch 001 identity, artifact, key, custody assumption, result, or authorization transfers into Epoch 002.
...
The repository still contains no canonical Epoch 002 materialization receipt and no accepted Epoch 002 materializer implementation. The future implementation identity reserved by the evidence schema is:

`scripts/materialize_track_a_epoch_002_unblinded_input.py`

That path is an identity contract only. This tooling PR does not create that implementation. A future real materialization event must bind an exact accepted materializer commit and blob before a receipt can validate.

## Established predecessor chain

Tooling-only validation now fails closed unless both accepted predecessors exist and remain semantically and historically intact:

1. `TRACK_A_EPOCH_002_DATASET_LOCK_RECEIPT.json` must exist, conform to the result-record schema, remain `PASS`, preserve all dataset-lock non-effects, and have exactly one immutable history event.
2. `TRACK_A_EPOCH_002_UNBLINDING_DECISION_RECORD.json` must exist, conform to the result-record schema, remain `PASS`, retain scope `CONTROLLED_MAPPING_RELEASE_OR_DECRYPTION_ONLY`, preserve its bounded non-effects, and have exactly one immutable history event.
3. The unblinding record must name the accepted dataset-lock record as its sole predecessor.
4. The unblinding immutable subject must bind the exact dataset-lock event commit and SHA-256 of the canonical dataset-lock receipt bytes.
5. The accepted unblinding event must descend from the accepted dataset-lock event.

These checks establish only predecessor integrity. They do not perform decryption or materialization.
...
Items 1–4 are established. Items 5–7 are not established by this tooling PR. No tooling PR may fabricate a materializer, retained evidence artifact, materialized input, or receipt.
...
For issue #632's refreshed tooling PR, acceptance requires all of the following:

- the accepted dataset-lock predecessor remains present and valid;
- the accepted bounded unblinding predecessor remains present, valid, and exactly bound to the dataset-lock receipt;
- both predecessor paths retain exactly one immutable history event;
...
At this tooling stage preserve:
