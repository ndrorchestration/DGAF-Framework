...
| P2 Execution Contract / Runtime | authenticated five-case runtime matrix on exact deployment | **PRIOR VERIFIED / CURRENT CANDIDATE OPEN** | fresh run bound to `c6157158…` and exact candidate deployment; prior run `33300481208` remains scoped to `303f4424…` |
...
| P6a Runtime/CORS | authenticated four-case CORS matrix on exact deployment | **PRIOR VERIFIED / CURRENT CANDIDATE OPEN** | fresh run bound to `c6157158…` and exact candidate deployment |
...
P2 remains **VERIFIED** for the exact runtime boundary exercised by authenticated workflow run `33300481208`, job `99227568599`, artifact `9728767844`, digest `sha256:cdbf23bf2a754034c9f5f5651b9242c22814669962a43bd59c409a0f7bf610a5`, candidate `303f4424d2198f0d0cf76305c589263dd1e417dc`, and deployment `dpl_FbPSc3K9VFWESXuUuWDepBKwKra8`.

All five required cases passed and the artifact records `all_pass = true`. This is retained as exact prior-candidate evidence. It is **not** evidence for the designated `c6157158…` candidate.
...
## Candidate-cycle requirement

The designated candidate is `c6157158bf0ee4840e99a381a4b99bd2febe2302`. A candidate-specific Vercel deployment is required before P2/P6a can be re-exercised. The current deployment observed for that commit was still `BUILDING` at inspection time, so no current-candidate runtime completion is claimed.

Because the designated candidate was created as a new candidate cycle, downstream evidence must bind directly to that SHA or to a formally documented equivalence rule that is accepted by the governing protocol. No historical P2/P6a, E2b, M6, P3, P4, P5, or P8 evidence is silently inherited merely because the candidate is descended from previously verified source.
...
9. P8 candidate-scoped analysis lock and closure.
...
**Prior P2/P6a VERIFIED at `303f4424…`. Current candidate `c6157158…` designated but not yet runtime-verified. P3–P6 evidence-gated. P8 OPEN / FAIL-CLOSED. P9 NOT EXECUTED. No freeze. No pilot authorization. Empirical N = 0.**
