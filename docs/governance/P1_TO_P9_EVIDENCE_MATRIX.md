...
|---|---|---|
| P1 Candidate Integrity | **CLOSED / VERIFIED** | exact apparatus/historical candidate/tree plus deployment-to-candidate binding retained |
| P2 Execution Contract / Runtime | **CLOSED / VERIFIED** | run `33730195621`; artifact `9883521704`; digest `sha256:5ca5bd3496c31f569a87338c1a0a3d93200e46106a5efda19d8269022adf696d`; scoped to historical candidate `7c1cc4bb…` |
...
| P5 Provenance / Reproducibility | **CLOSED / VERIFIED** | exact historical candidate identities, deterministic configuration, exact-candidate reproduction evidence, authoritative binding merge `2e325acd…`, and post-merge Governance CI / pre-authorization security PASS; see `P5_PROVENANCE_REPRODUCIBILITY_ATTESTATION_2026-09-05.md`; protocol/schema identities must be reverified against the final v0.7.6 descendant |
...
| P6a Runtime / CORS | **CLOSED / VERIFIED** | run `33728695806`; artifact `9882965299`; digest `sha256:527145195518f7ed147507e02b3ed7cdc4bd9be0c547645dedd094a4f4d3340f`; scoped to historical candidate `7c1cc4bb…` |
...
Historical candidate `7c1cc4bb78025b21501b6f790bf55f4b5e3bbdc8`, tree `586c00d6…`, is exact-bound to production deployment `dpl_8MsufVUMXHMGqx9d1dcK9va5EWUA`. This establishes deployment identity for the historical candidate, not current-main production health or efficacy. The final v0.7.6 descendant has not yet been designated; see Issue #309.

### P2 — authenticated runtime matrix

Run `33730195621` completed all five predeclared exact-candidate runtime cases successfully. Artifact `9883521704` is candidate/deployment bound. The expected HTTP 503 / `BLOCKED` valid-request result is fail-closed contract behavior because live audit state is not wired; it is not application-health evidence. This evidence is scoped to historical candidate `7c1cc4bb…` and is not transferable to a later candidate without explicit provenance transition.
...
Exact historical candidate identities:
...
P5 closure is not efficacy evidence and does not imply freeze or authorization. The protocol/schema/analysis-configuration identities above are bound to the historical candidate `7c1cc4bb…`; they must be reverified against the final v0.7.6 descendant designated by #309 before they can be treated as current-candidate evidence.
