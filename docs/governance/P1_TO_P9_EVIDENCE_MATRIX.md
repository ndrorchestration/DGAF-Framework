...
PR #308 changed the apparatus to protocol v0.7.6 / artifact schema 1.1. Therefore the prior `7c1cc4bb…` candidate remains provenance-valid for its exact evidence boundary but is no longer eligible as the final freeze/N=1 candidate. Issue #309 governs final-candidate reconstruction and evidence transfer/reverification.

## Predicate matrix

| Predicate | Current state | Evidence / remaining boundary |
|---|---|---|
| Historical P1/P2/P3/P5/P6/P6a evidence | **CLOSED / VERIFIED AT `7c1cc4bb…` SCOPE** | remains valid provenance; does not automatically transfer to final v0.7.6 candidate |
| Final candidate identity | **NOT DESIGNATED** | exact immutable v0.7.6-descended SHA/tree required under #309 |
| P4 Security / Blinding | **OPEN / PROCEDURE REVISED / OPERATION NOT EXECUTED** | canonical procedure accepts Mode H, I, or T; no real custody instance/no-unilateral-access evidence exists yet |
| P7 Scientific Target | **ADOPTED / FINAL BINDING OPEN** | final exact tuple blocked by final candidate identity plus actual P4-A custody evidence |
| P8 Analysis Lock | **OPEN / FAIL-CLOSED** | immutable freeze has not been created/verified |
| P9 Independent Verification | **NOT EXECUTED / OPEN** | fresh independent verification of final frozen chain remains required |

## Historical exact-scope evidence summary

### P1 — candidate/deployment identity

Historical evidence candidate `7c1cc4bb78025b21501b6f790bf55f4b5e3bbdc8`, tree `586c00d6…`, is exact-bound to production deployment `dpl_8MsufVUMXHMGqx9d1dcK9va5EWUA`. This establishes historical deployment identity for that evidence boundary, not current-main equivalence or final-candidate identity.

### P2 — authenticated runtime matrix

Run `33730195621` completed all five predeclared exact-candidate runtime cases successfully. Artifact `9883521704` is bound to `7c1cc4bb…` and `dpl_8Msuf…`. This remains valid historical runtime evidence and must be reclassified under #309 before use toward final-candidate closure.

### P3 — artifact contract

Run `33939955138` verified candidate/tree identity, canonical matrix/schema/cardinality constraints, deterministic contract behavior, duplicate rejection, and fail-closed unauthorized pilot behavior for the pre-v0.7.6 candidate boundary. P3 remains structural/contract evidence only.

### P4 — synthetic evidence plus independently enforceable custody procedure

Synthetic mock-key blinding/bijection/leakage/freeze-order controls pass. The canonical procedure is `docs/governance/P4_INDEPENDENT_BLINDING_CUSTODY_PROCEDURE.md`.
...
Historical exact identities include:
...
These identities are not silently rewritten as v0.7.6 evidence. Issue #309 decides what requires fresh binding or reproduction for the final candidate.

### P6 — durable custody

The retained P3/P5 and P4 evidence ZIPs were independently archived, retrieved, and re-hashed; retrieved SHA-256 values matched the original GitHub artifact digests. Scope remains byte-integrity/retention for the specified historical evidence set.

### P6a — authenticated CORS matrix

Run `33728695806` completed the four exact-candidate CORS cases successfully for `7c1cc4bb…` / `dpl_8Msuf…`. This evidence remains scoped to that tested endpoint, deployment, environment, and origins.
...
The consolidated control-state anchor `89be386b…` remains a valid control/provenance anchor; it is not a final-candidate designation.
...
4. Close real P4-A custody for the exact final candidate/apparatus.
