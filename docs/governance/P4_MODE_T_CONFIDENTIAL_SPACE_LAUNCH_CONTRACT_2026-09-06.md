...
Therefore `PASS_PREDECLARED_ONLY` means only that a proposed launch declaration is internally consistent with the reviewed launch contract.

## Required real-admission reconciliation

Before any future Mode-T PASS can rely on this declaration, a real authenticated run must independently prove at least:
