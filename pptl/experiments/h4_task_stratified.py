...
def evaluate_h4(summary: list[dict[str, Any]]) -> str:
    ...
    on TASK3 at noise=0.30, while remaining within the non-dominance
    tolerance of both comparison triads on TASK1 and TASK2.

    Missing required summary cells are an error. An incomplete summary must
    never be converted into a confirmatory H4 verdict by implicit zero values.
    """
    ...
    lines = [
        ...
        "H4-Part2 (C not dominant on TASK1/TASK2): "
        f"{'CONFIRMED' if h4_part2 else 'REJECTED'}",
        "",
