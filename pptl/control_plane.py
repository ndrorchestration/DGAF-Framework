...
class TaskState(str, Enum):
    RECEIVED="RECEIVED"; PREFLIGHT="PREFLIGHT"; ADMITTED="ADMITTED"; EXPANDING="EXPANDING"; EVALUATING="EVALUATING"; MERGE_READY="MERGE_READY"; COMMIT_READY="COMMIT_READY"; ESCALATED="ESCALATED"; TERMINATED="TERMINATED"
...
TaskState.ESCALATED:{TaskState.TERMINATED},TaskState.TERMINATED:set()}

class ControlPlaneViolation(RuntimeError): pass
...
class ControlTask:
    ...
    def snapshot(self)->dict[str,object]:
        return {"task_id":self.task_id,"state":self.state.value,"depth":self.depth,"envelope_trace":self.envelope.trace_id,"parent_trace":self.envelope.parent_trace_id}
...
class ControlPlane:
    ...
    def __init__(self, *, tgl_runner:Callable[...,Any]|None=None)->None:
        self.tgl_runner=tgl_runner; self.state_registry=StateRegistry(); self.branches=BranchRegistry(); self.tasks={}; self.ledgers={}; self.events=[]; self._lineage_active={}; self._lineage_limits={}
    ...
    def evaluate_turn(self,task_id:str,input_text:str,context:dict[str,Any]|None=None)->Any:
        if self.tgl_runner is None: raise ControlPlaneViolation("no TGL runner configured")
    ...
    def terminate(self,task_id:str)->None:
        ...
        if task.concurrency_acquired:
            self.ledgers[task_id].release_concurrency(); lineage=task.lineage_id or task.envelope.trace_id; self._lineage_active[lineage]=max(0,self._lineage_active.get(lineage,0)-1); task.concurrency_acquired=False
    def create_child(self,parent_id:str,*,task_id:str,trace_id:str,authority_scope:set[str],permitted_tools:set[str],data_classes:set[str],envelope_budget:ResourceBudget)->ControlTask:
        ...
        if parent.state not in {TaskState.ADMITTED,TaskState.EXPANDING,TaskState.EVALUATING}: raise ControlPlaneViolation("child creation requires an active parent task")
        if parent.depth+1>parent.envelope.budget.max_depth: raise ControlPlaneViolation("child exceeds maximum recursion depth")
        ...
        if self.state_registry.contains(child.snapshot()): raise ControlPlaneViolation("repeated orchestration state")
        self.state_registry.observe(child.snapshot()); self.submit(child); return child
    def register_branch(self,branch:BranchRecord)->None:
        self.branches.add(branch); self.events.append({"event":"BRANCH_RECORDED","branch_id":branch.branch_id,"policy_verdict":branch.policy_verdict,"merge_status":branch.merge_status})
    ...
    def _task(self,task_id:str)->ControlTask:
        ...
        except KeyError as exc:raise KeyError(task_id) from exc
