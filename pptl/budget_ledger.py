...
class BudgetLedger:
    ...
    def _committed_or_reserved(self) -> Consumption:
        return self._add(self.consumed, self.reserved)

    def remaining(self) -> Consumption:
        used = self._committed_or_reserved()
        limits = {
            ...
            "elapsed_ms": self.budget.max_elapsed_ms,
            "rounds": self.budget.max_rounds,
    ...
    def acquire_concurrency(self, slots: int = 1) -> None:
        ...
        if self.active_concurrency + slots > self.budget.max_concurrency:
    ...
    def reserve(self, amount: Consumption) -> None:
        candidate = self._add(self._committed_or_reserved(), amount)
        if not self._fits(self.budget, candidate):
            raise BudgetExceeded("resource reservation exceeds budget")
    ...
    def consume(self, amount: Consumption) -> None:
        candidate = self._add(self._committed_or_reserved(), amount)
        if not self._fits(self.budget, candidate):
            raise BudgetExceeded("resource consumption exceeds budget")
        self.consumed = self._add(self.consumed, amount)
