...
| Recovery Score | R_c | Tribunal exit criterion; live recovery wiring remains evidence-pending |
...
This bounds φ ∈ [1.0, 1.8] by construction when the logistic definition is applied as `σ(x) = 1 / (1 + e^(-x))`. φ is intended to be derived from measurable agent behaviors rather than asserted.
...
- **P-29 (Sentinel):** Tribunal archetype (φ > 1.80) is intended to fire P-29 `risk_block` via DemiJoule
- **P-33 (PDMAL):** Governance Momentum (M) is documented as a PDMAL substrate relationship; this does not establish mutual implementation or validation
- **P-38 (Circuit-Breaker):** AHG Tribunal mode is intended to emit a circuit-breaker OPEN signal on φ > 1.80

## Implementation / Evidence Status

| Component | File | Status | Evidence boundary |
|---|---|---|---|
| AHG full spec | `docs/theory/AHG_ARCHITECTURE.md` | ✅ Defined | Specification, not independent validation |
| Pattern card | `patterns/P-42_AHG.md` | ✅ Current | This status-synchronized card |
| Conductor implementation | `components/ahg_conductor.py` | 🟢 Implemented | Core φ/regime/Tribunal architecture present |
| Sidecar Monitor | `components/ahg_sidecar.py` | 🟢 Implemented/reviewed | v1.5 architecture reviewed; production operation not attested here |
| Herald trace | `components/ahg_herald_trace.py` | 🟢 Implemented/reviewed | Architecture review identifies implementation |
| Heartbeat schema | `schemas/ahg_heartbeat.json` | 🟢 Implemented/reviewed | Schema review available |
| Conductor test suite | `tests/test_ahg_conductor.py` | 🟢 Implemented | Unit/test harness present |
| Eval suite | `tests/dgaf_eval_suite.py` | 🟡 Stub/live split | AHG tasks 6–8 have deterministic stub calibration; live conductor run required |
| Tribunal `R_c` recovery loop | `components/ahg_conductor.py` / related wiring | 🟡 Evidence pending | Do not represent live recovery as independently verified |
| MPHG optimizer | `components/ahg_mphg.py` | 🔴 Roadmap | v2.0 scope |

## Evidence Standard

`IMPLEMENTED` means source code or an executable harness exists. `VERIFIED` requires reproducible execution evidence for the specific claim. `ATTESTED` requires a dated run or equivalent provenance. A test of a stub is evidence for the stub behavior, not proof of the production capability it represents.

## Current P-42 Gate

The architecture review records P-42 as approved/close-eligible **only after a successful live evaluation run**. Therefore the current state remains **partially implemented / live evidence pending**.
